...
data(FANG)
groups <- c("GOOG","FB", "AMZN","GOOG")
groupFANG <- FANG %>%
    filter(symbol == groups[1])%>%
       arrange(date)
...
targets = c("open","close", "high", "low", "volume", "adjusted")
time_decompose_methods = c("stl","twitter")
#maybe make a trend/frequency slider?

groupFANG %>%
  time_decompose(target = targets[1],method = "stl", message=FALSE)
```
...
targets = c("open","close", "high", "low", "volume", "adjusted")
time_decompose_methods = c("stl","twitter")
#maybe make a trend/frequency slider for decompose?

########################################################

#anomalize
anomalize_methods= c("iqr","gesd")
#range from 0.025 to 0.1
alpha_value = 0.05
#slider from 0.05 to 0.5
max_anom_values = 0.2

groupFANG %>%
  time_decompose(target = adjusted, method = time_decompose_methods[1], message=FALSE)%>%
  anomalize(target = remainder, method = anomalize_methods[1],alpha = alpha_value, max_anoms = max_anom_values)
...
targets = c("open","close", "high", "low", "volume", "adjusted")
time_decompose_methods = c("stl","twitter")
#maybe make a trend/frequency slider for decompose?possibly for anomalize?
...
#color slider
color_not_anomalies = "#2c3e50"
color_anomalies = "#e31a1c"
bands = "grey70"
#slider from 0 to 1
dots_alpha = 1
anomaly_alpha = 1
...
groupFANG %>%
  ...
  anomalize(target = remainder, method = anomalize_methods[1],alpha = alpha_value, max_anoms = max_anom_values) %>%
  time_recompose()%>%
  plot_anomalies(time_recomposed = TRUE, color_no = color_not_anomalies, color_yes = color_anomalies, 
                 fill_ribbon = bands, alpha_dots = dots_alpha, alpha_circles = anomaly_alpha, alpha_ribbon = bands_alpha, 
                 size_dots = dot_size, size_circles = anomaly_size)
...
##############################
pos = c("top", "bottom", "left", "right") #only for anomaly decompositon
######################
bands_alpha = 1 #only for plot anomalies
bands = "grey70" #only for plot anomalies
#######################
groupFANG %>%
  time_decompose(target = adjusted,method = "stl",message=FALSE)%>%
  anomalize(target = remainder)%>%
plot_anomaly_decomposition(ncol=2,color_no = color_not_anomalies, color_yes = color_anomalies, 
                          alpha_dots = dots_alpha, alpha_circles = anomaly_alpha,  
                          size_dots = dot_size, size_circles = anomaly_size, strip.position = pos[4])
```
