...
make_phrase <- function(num, item, verb, adjective, location){
  ...
  phrase <- str_replace(phrase,pattern = as.character(num), replacement = num_strings)
  phrase <- str_trim(phrase)
  return(phrase)
}

make_phrase(xmas$Day,xmas$Plural.Gifts,xmas$Verb,xmas$Adjective,xmas$Location)
```
...
for (i in 1:12){
  ...
  for(j in i:1){
    if(i==1){
      print(paste0(xmas$phrases[j],"."))
    }
    else if(i==2 & j== 2){
      print(xmas$phrases[j])
    }
    else if(i!=1 & j == 1){
      print(paste0("and ",xmas$phrases[j],"."))
    }
    else{
      print(paste0(xmas$phrases[j],", "))
    }
...
sing_line <- function(xmas, num){
  ...
  phrase <- paste("On the", xmas$Day.in.Words[num], "day of Christmas, my true love sent to me, ")
...
sapply(vec,function(x) sing_line(num =x,xmas))
```
