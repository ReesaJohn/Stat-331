...
# load packages
source("stat331_shared/Scripts/packageLoaders.R")
...
# Barplots to summarize categorical variables

survey %>%
  ggplot(aes(x = Year, fill = Year)) + geom_bar() +
  ggtitle("School year of 331 students")

# Histograms to summarize numeric variables

survey %>%
  ggplot(aes(x = CS.Classes)) + geom_histogram(stat = "count")
...
survey %>% 
  ggplot(aes(x = Kardashian, fill = Kardashian)) + facet_wrap(~Political) + geom_bar(postion = "fill") + 
  ggtitle("Kardashian preference (as percent) by Political Party")


survey %>% 
  ggplot(aes(x = Political, fill = Hot.Dog)) + facet_wrap(~Major) + geom_bar() +
  ggtitle("Hot dog opinions by political party and major")
...
cal_poly_sentiment <- survey_orig %>% 
  ...
  inner_join(get_sentiments("bing"), by = c("Cal.Poly.Word" = "word"))
