...
```{r, echo = FALSE}
  titanic %>%
  ggplot(aes(x = Survived,fill = Survived)) + geom_bar()+scale_fill_brewer(palette="Blues")
...
```{r}
num_passengers_classSex <- titanic %>%
  group_by(Pclass,Sex)
  

num_passengers_classSex

ggplot(num_passengers_classSex,aes(x=Pclass,fill = factor(Sex)))+geom_bar(position = "dodge")+  scale_color_discrete("Sex")

numberOfBoth <- titanic %>%
  group_by(Pclass,Sex) %>%
  tally()
numberOfBoth
```
...
There were 3 people with the most expensive ticket
...
```{r}
titanic %>%filter(Pclass==2 & Age > 30 & Sex == "female") %>% top_n(1,desc(Fare))
...
The second plot is best because it shows that there are more males than females and that most of the females lived and most of the males died.
...
A woman is far more likely to survive than a man.
...
```{r}
titanic %>% ggplot(aes(x = Survived, fill = factor(Pclass))) + geom_bar(position = "dodge")

aplot = titanic %>%group_by(Sex,Pclass)

ggplot(aplot,aes(x=Pclass,fill=Survived))+geom_bar(position="dodge",alpha = 0.5)
titanic %>% filter(Sex=="male")%>%ggplot(aes(x=Pclass,fill=Survived))+geom_bar(position = "dodge",alpha = 0.5)
```
